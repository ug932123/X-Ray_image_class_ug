...
public class Login extends AppCompatActivity {
    ...
    private static final String SERVER_URL = "http://national-pleasantly-earwig.ngrok-free.app/login"; // Change this to your server URL
                        ...
                        try {
                            ...
                            if (jsonResponse.has("Session")) {
                                ...
                                if(sessionKey.equals("True"))
                                {
                                    ...
                                    startActivity(intent);
                                    finish();
                                } else if (sessionKey.equals("False")) {
                                    runOnUiThread(new Runnable() {
                                        @Override
                                        public void run() {
                                ...
                                runOnUiThread(new Runnable() {
                                    ...
                                    public void run() {
                                        login_progress.setVisibility(View.INVISIBLE);
                                        Toast.makeText(Login.this, "No session key", Toast.LENGTH_SHORT).show();
                                    }
                                });
                            }
                        } catch (JSONException e) {
                            ...
                            runOnUiThread(new Runnable() {
                                @Override
                                public void run() {
                                    Toast.makeText(Login.this, "Error parsing JSON response", Toast.LENGTH_SHORT).show();
                                }
                            });
                        }
                    }
