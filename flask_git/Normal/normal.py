...
for i in range(1, 22):
    ...
    try:
        ...
        output_confidence = max(confidence[0]) * 100
        output_confidence = round(output_confidence, 2)
        
        true_label = 0  # Assuming that all images in 'Normal' folder are labeled as 'Normal'
        true_labels.append(true_label)
        
        print(categories[predicted_label])
        print(output_confidence)
        print(f'Normal/l{i}.jpg: {prediction}')
        
        data_to_append = [[f'l{i}.jpg', categories[predicted_label], output_confidence]]
...
append_to_excel(file_path_sheet, 'Metrics_Nor', metrics_data)
...
plt.savefig('Nor_confusion_matrix.png')
plt.show()
